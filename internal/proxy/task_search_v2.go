...
func (t *searchTaskV2) Execute(ctx context.Context) error {
	...
	for _, shard := range shards {
		...
		t.runningGroup.Go(func() error {
			log.Debug("proxy starting to search one shard",
...
func (t *searchTaskV2) searchShard(ctx context.Context, leaders *querypb.ShardLeadersList) error {
	...
	err := t.RoundRobin(search, leaders)
	if err != nil {
...
func (t *searchTaskV2) RoundRobin(query func(UniqueID, types.QueryNode) error, leaders *querypb.ShardLeadersList) error {
	var (
		err     = errBegin
		current = 0
		qn      types.QueryNode
	...
	for err != nil && current < replicaNum {
		...
		if err != nil {
			log.Warn("fail to get valid QueryNode", zap.Int64("nodeID", currentID), zap.Error(err))
			current++
			continue
		}
...
func (t *searchTaskV2) checkIfLoaded(collectionID UniqueID, searchPartitionIDs []UniqueID) bool {
